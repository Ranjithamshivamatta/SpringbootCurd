...
public class UserServiceImpl implements UserService {
	 ...
	 */
	/*
	 * @Override public String jwtToken(String secretKey, int id) { long nowMillis =
	 * System.currentTimeMillis(); Date now = new Date(nowMillis);
	 * 
	 * JwtBuilder builder =
	 * Jwts.builder().setSubject(String.valueOf(id)).setIssuedAt(now)
	 * .signWith(SignatureAlgorithm.HS256, base64SecretBytes);
	 * System.out.println("jwt token :" + builder.compact()); String token =
	 * builder.compact();
	 * 
	 * return token; }
	 */
	...
	/*
	 * @Override public int tokenVerification(String token) { // This line will
	 * throw an exception if it is not a signed JWS (as expected) if
	 * (StringUtils.isEmpty(token)) { } Claims claims =
